...
bool Level::loadFromFile (const std::string filename)
{
	...
	while (!Datei.eof())
	{
		...
		switch (line[0])
		{
			...
			case 'M':
			{
				...
				uv[t].push_back (glm::vec2(ux[0], uy[0]));
				uv[t].push_back (glm::vec2(ux[1], uy[1]));
				uv[t].push_back (glm::vec2(ux[2], uy[2]));
				normals[t].push_back (glm::vec3(nx, ny, nz));
				normals[t].push_back (glm::vec3(nx, ny, nz));
				normals[t].push_back (glm::vec3(nx, ny, nz));
	...
	glBufferData (GL_ARRAY_BUFFER, mVertices.size() * sizeof(glm::vec3) + mUV.size() * sizeof(glm::vec2) + mNormals.size() * sizeof(glm::vec3), &mVertices[0], GL_STATIC_DRAW);
